...
<div class="w3-top">
    ...
    <div class="w3-col s3">
      ...
      <a href="contactus.jsp" class="w3-button w3-block w3-black">Contact Members</a>
		  	...
		  	<% ArrayList<BalUsers> balanceList = new ArrayList <BalUsers>(); 
		  	balanceList = (ArrayList<BalUsers>)request.getAttribute("balanceList");
						    	      ...
						    	      for( int i = 0; i<balanceList.size();i++) {
												...
												//listUsers.get(i).getSavings();
												//totalAmountReceived = listUsers.get(i).getLoanamount();
												//out.print("<td )>"  + "$" +listUsers.get(i).getInterest() + "</td>" + "</tr >");
												
												totalAmountReceived = totalAmountReceived+ balanceList.get(i).getSavings();
												totalLoanAmount = totalLoanAmount+ balanceList.get(i).getLoanamount();
				...
				<h3 class="btn btn-success"> Total Amount Available <span style = "font-weight : bold">$${available} </span> </h3>
												...
												<tbody class="success">
													...
													<% ArrayList<LoanUsers> loanUsers = new ArrayList <LoanUsers>(); 
						 							...
						 							loanUsers = (ArrayList<LoanUsers>)request.getAttribute("loanUsers");
						 							 if ((boolean)request.getAttribute("noDataB")) {
													    for ( int i = 0; i < loanUsers.size(); i++ ) {
													        %>
													        <tr>
													       <td> <%	out.print( loanUsers.get(i).getId() ); %> </td>
													       <td><% out.print( loanUsers.get(i).getFirstname()); %> </td>
													       <td> <% out.print(loanUsers.get(i).getEmail()); %> </td>
															 <td> <% out.print(loanUsers.get(i).getAmount()); %></td>
															  <td> <% out.print(loanUsers.get(i).getMessage()); %></td>
															  <td> <a href = "ApprovalRequest.java" ><button onclick="myVar = setTimeout(myFunction, 3000)"type="button" class="btn btn-primary btn-sm">Approve </button>	</a> </td>
															  <td> <a href = "index.jsp" ><button  onclick="myVar = setTimeout(myFunction1, 3000)" type="button" class="btn btn-danger">Decline</button> </a></td>
													        </tr>
													        <%
													    }
						 							 }
